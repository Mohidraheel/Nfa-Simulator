...
main()
{
    ...
    for (int i = 0; i < newtransitions.size(); i++)
    {
        ...
        {
            char c = alphabet[j];
            if (newtransitions[i].count(c))
            {
                cout << "{";
                for (int k = 0; k < newtransitions[i][c].size(); k++)
                {
                    if (k > 0)
                    {
                        cout << ",";
                    }
                    cout << newtransitions[i][c][k];
                }
                cout << "}" << setw(10 - (int)newtransitions[i][c].size() * 2) << "";
            }
            else
            {
                cout << left << setw(10) << "-";
    ...
    unordered_set<int> currents = {starts};
    for (int i = 0; i < teststring.size(); i++)
    {
        char c = teststring[i];
        unordered_set<int> nexts;
        vector<int> currentstatesvec(currents.begin(), currents.end());
        for (int j = 0; j < currentstatesvec.size(); j++)
        {
            int state = currentstatesvec[j];
            if (newtransitions[state].count(c))
            {
                for (int k = 0; k < newtransitions[state][c].size(); k++)
                {
                    nexts.insert(newtransitions[state][c][k]);
                }
            }
        }
        currents = nexts;
    ...
    bool accepted = false;
    vector<int> currentstatesvec(currents.begin(), currents.end());
    for (int i = 0; i < currentstatesvec.size(); i++)
    {
        int state = currentstatesvec[i];
        if (newaccepts[state])
        {
            accepted = true;
            break;
